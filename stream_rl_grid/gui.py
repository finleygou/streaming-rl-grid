...
from .config import AgentConfig, AppConfig, EnvironmentConfig, PROFILES, TrainingConfig
...
class TrainingPanel:
    ...
    def _build_layout(self) -> None:
        ...
        self._add_entry(env_tab, "Context maps", "num_contexts", 4)
        self._add_entry(env_tab, "Seed", "seed", 5)
        self._add_entry(env_tab, "Goal reward", "reward_goal", 6)
        self._add_entry(env_tab, "Collision reward", "reward_collision", 7)
        self._add_entry(env_tab, "Step reward", "reward_step", 8)
        self._add_entry(env_tab, "Max wind strength", "max_wind_strength", 9)
        self._add_entry(env_tab, "Wind/reward period", "wind_period", 10)
        self._add_entry(env_tab, "Goal move interval", "target_move_interval", 11)
        self._add_entry(env_tab, "Context switch interval", "context_switch_interval", 12)
        preview_row = ttk.Frame(env_tab)
        preview_row.grid(row=13, column=0, columnspan=2, sticky="ew", padx=6, pady=8)
        ...
        self.stop_button = ttk.Button(button_box, text="Stop + save", command=self.stop_training, state=tk.DISABLED)
    ...
    def _read_config(self) -> AppConfig:
        env = EnvironmentConfig(
            ...
            num_contexts=int(self.variables["num_contexts"].get()), profile=self.variables["profile"].get(),
        ...
        expected_maps = env.num_contexts if env.profile in ("hidden_context", "combined") else 1
        if self.preview_maps is not None and len(self.preview_maps) == expected_maps:
            if all(len(layout) == env.obstacle_count for layout in self.preview_maps):
    ...
    def generate_preview(self) -> None:
        ...
        try:
            ...
            config.environment.context_maps = None
            ...
            snapshot = {
                "agent_state": environment.agent_state, "goal": environment.goal,
    ...
    def _on_grid_click(self, event: tk.Event) -> None:
        if self.worker and self.worker.is_alive() or not self.preview_maps:
            return
        ...
        if self.selected_obstacle is None:
            ...
            if point not in layout:
                ...
                self.selected_obstacle = None
        snapshot = {
            "agent_state": (-1, -1), "goal": (-1, -1), "obstacles": sorted(layout),
        ...
        try:
            while not self.stop_event.is_set():
                ...
                if self.pause_event.is_set():
                    time.sleep(0.05)
                    continue
                snapshot = self.trainer.run_steps(self.trainer.config.training.ui_update_steps)
                self.messages.put(("snapshot", snapshot))
            path = self.trainer.save()
            self.messages.put(("stopped", path))
        except Exception as exc:
            try:
                path = self.trainer.save()
            except Exception:
    ...
    def stop_training(self) -> None:
        self.stop_event.set()
        self.status_var.set("Stopping after the current streaming update...")
        ...
        try:
            while True:
                ...
                if kind == "snapshot":
                    ...
                    self.status_var.set("Stopped and saved: %s" % payload)
    ...
    def _on_close(self) -> None:
        if self.worker and self.worker.is_alive():
            if not messagebox.askyesno("Stop training", "Stop and save the current training before closing?"):
                return
            self.stop_event.set()
